...
- **Full-vocabulary translation complete** (2026-07-25) — `Vocabulary` struct (`src/translations/mod.rs`) parsed from a sectioned `[keywords]/[types]/[builtins]/[methods]/[messages]` file format, threaded as `Rc<Vocabulary>` through `Lexer`/`Parser`/`Resolver`/`TypeChecker`/`Interpreter` via `with_vocabulary` constructors. CLI: `--vocabulary <path>` (`--keywords <path>` kept as alias). `src/translations/messages.rs` routes the large majority of lexer/parser/resolver/interpreter error strings through `Vocabulary::msg`, with English fallback for anything not yet catalogued (typechecker/diagnostics message conversion not swept). `translations/pt.vocab` fills all 5 sections; `examples/translations/hello_pt.yara` rewritten fully Portuguese (types/builtins/methods too, not just keywords); new `examples/errors/runtime_error_pt.yara` + golden demonstrate a localized runtime error message. `tests/run_examples.rs` and `tests/error_output.rs` updated to thread `Vocabulary` (not just a keyword map) so Portuguese-vocabulary examples type-check/run/render correctly under test. Docs: root `CLAUDE.md` TODO, `docs/syntax.md` Translation section, `docs/architecture.md` pipeline note.
