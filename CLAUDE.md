...
- **Configurable keyword translation** (i18n for keywords): a compile-time config (e.g. a `--keywords <lang>` flag or a `yara.toml`/JSON keyword map file) letting `if`/`while`/`class`/`def`/etc. be written in another language (e.g. Portuguese `se`/`enquanto`/`classe`/`funcao`) while the underlying `TokenKind` stays the same. Sketch:
  - Cleanest seam: `src/lexer/mod.rs`'s `read_ident_or_keyword` currently hardcodes the English keyword strings in one `match`. Generalize it to consult a `keyword_map: &HashMap<String, TokenKind>` passed into `Lexer::new` (built from a bundled default English map, or loaded from a user-supplied translation file) instead of a hardcoded match — every other stage (parser/typechecker/interpreter) is already keyword-string-agnostic since they only ever see `TokenKind`, so this is lexer-only in scope.
  - `main.rs` gains a flag/config to pick which keyword map to load; ships at least one working example translation (e.g. Portuguese) under a `translations/` or `i18n/` folder as proof it round-trips.
  - Scope explicitly excludes translating identifiers/strings/error messages — only the fixed set of reserved words. Error messages staying in English (or not) is a separate, unscoped decision.
  - Not started — smaller than the pointers/GC item, good candidate to pick up first.
