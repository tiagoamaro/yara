...
**Next-milestones plan** (`docs/plan-next-milestones.md`) — Phase 1 done: imported-file snippet fix via `diagnostics::SourceMap` virtual-line ranges (resolver shifts spliced ASTs, `render_with_map` maps back to file + local line; single-file output unchanged). Phase 2 done: definite-assignment soundness fix — typechecker requires every class instance-var to be assigned in `initializer`, reported at field declaration with flow-insensitive walk (`check_fields_assigned_in_initializer`); closes the old soundness gap. Golden error-output tests (`tests/error_output.rs` vs `tests/golden/*.stderr`) now lock the byte-identical-errors invariant automatically — regenerate a golden with `cargo run -- run examples/errors/<name>.yara 2> tests/golden/<name>.stderr` after an intentional format change. Structure work done too: CI workflow (fmt + test, Rust 1.97.1) and the parser/typechecker/interpreter submodule split (each stage now has exprs/stmts + calls/classes files; mod.rs keeps public types, dispatch entry points, and tests). 94 unit + 4 integration tests, `cargo fmt` clean. Remaining phases + order: see the Progress section at the top of `docs/plan-next-milestones.md` (next up: Phase 3a, pointers + heap).
