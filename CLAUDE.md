...
**Next-milestones plan** (`docs/plan-next-milestones.md`) — Phase 1 done: imported-file snippet fix via `diagnostics::SourceMap` virtual-line ranges (resolver shifts spliced ASTs, `render_with_map` maps back to file + local line; single-file output unchanged). Golden error-output tests (`tests/error_output.rs` vs `tests/golden/*.stderr`) now lock the byte-identical-errors invariant automatically — regenerate a golden with `cargo run -- run examples/errors/<name>.yara 2> tests/golden/<name>.stderr` after an intentional format change. 91 unit + 4 integration tests, `cargo fmt` clean.
