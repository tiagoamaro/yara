...
- **Full-vocabulary translation**: extend the keyword-translation system (`--keywords`, `src/translations/`) beyond keywords to *all* user-facing terms — type names (`Integer`, `Boolean`, `Ptr`, ...), boolean literals (`true`/`false`), builtin function names (`len`/`push`/`alloc`/`deref`/..., `print`), primitive methods, and error-message prose, so a learner can write and debug fully localized programs (e.g. all-Portuguese including `Inteiro`, `verdadeiro`, `tamanho`). In progress — plan at `~/.claude/plans/let-s-plan-how-we-cozy-volcano.md`; a sectioned `[keywords]/[types]/[builtins]/[methods]/[messages]` vocabulary file format and a `Vocabulary` struct have landed, threading + hook points + message-catalog conversion + docs/examples still open. **Once complete, update every mermaid diagram touched by the change** (`docs/architecture.md`, stage-module `///` flowcharts) and every affected folder's CLAUDE.md — not just this TODO entry.
