...
- `pointers/` — manual memory management and garbage collection demo (`alloc`/`deref`/`set_deref`/`free`/`collect()` builtins). `basic.yara` exercises allocation, dereferencing, and explicit freeing. `leak.yara` demonstrates allocations never freed (allowed in manual mode). `gc.yara` shows a manual-vs-GC comparison, leaking allocations reclaimed by `collect()`; see its own `CLAUDE.md`.
