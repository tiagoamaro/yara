module Yara
  ...
  module RustFormat
    ...
    # base-10 exponent of the first digit: 0.015 gives `["15", -2]`. Tries
    # increasing `%e` precision until parsing the text gives `value` back.
    #
    # @param value [Float] finite and nonzero
    # @return [Array(String, Integer)]
    def self.shortest_digits(value)
      magnitude = value.abs
      (0..16).each do |precision|
        text = format("%.#{precision}e", magnitude)
        next unless text.to_f == magnitude

        mantissa, exponent = text.split("e")
        digits = mantissa.delete(".")
        digits = digits[0...-1] while digits.size > 1 && digits.end_with?("0")
        return [digits, exponent.to_i]
      end
    end
