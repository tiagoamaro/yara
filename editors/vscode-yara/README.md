...
Syntax highlighting for `.yara` files. A TextMate grammar only — no language server, no diagnostics, no folding, no autocomplete. It colors the source; the compiler (`cargo run --manifest-path rust/Cargo.toml -- run <file>` from the repo root) is what actually catches errors, with source-snippet-and-caret rendering (see `rust/src/main.rs`).
...
TextMate grammars are regex-based pattern matching over text, not a real parser — they don't know Yara's actual grammar the way `rust/src/parser/` does. So:
