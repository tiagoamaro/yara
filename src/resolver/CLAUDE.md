...
## Design
- `mod.rs` carries a `///` mermaid flowchart on `resolve_imports` showing the recursive splice/cycle-detection/virtual-line-shift flow — keep it in sync if the resolution algorithm changes.
...
- Every imported file is lexed and parsed with the *same* `Rc<Vocabulary>` as the entry file (`Lexer::with_vocabulary(&source, vocab.clone())` + `Parser::with_vocabulary(tokens, vocab.clone())`, threaded as a `&Rc<Vocabulary>` parameter through `resolve_imports`/`resolve`) — a localized program's imports are written in the same localized vocabulary, not forced back to English, and lexer/parser error prose inside an imported file is localized too.
- `ResolveError`'s own messages (`cannot resolve import`, `import cycle detected at`, `cannot read imported file`, `lex error in`, `parse error in`) are built via `vocab.msg("resolve/...", &args)` rather than inline `format!` — the wrapped `{e}` (an `io::Error`/`LexError`/`ParseError`'s own `Display`) is passed through as an opaque string argument, so only the resolver's *own* wrapper prose is localizable, not the wrapped inner error's message shape.
