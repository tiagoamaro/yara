...
- Function return-type check compares the declared return type against the type of the body's final expression (implicit last-expr return) or an explicit trailing `return expr` — see `check_body_return_type`.
